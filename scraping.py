...
def fetch_daily_pdf():
    ...
    pdf_link = PDF_PATH.replace("pasta=ANO", current_year)
    ...
    # Considerei 20365 um limite bom para o nº de publicações que pode haver no ano. (Limite = 365)
    for i in range(20001, 20365):

        updated_link = pdf_link.replace("20ID", str(i))

        # Buscando o pdf no link
        res = requests.get(updated_link)

        if res.status_code != 200:
            continue

        # Conseguindo o link correto, o loop é terminado
        break
